...
export default function ConventionalRefiIntel() {
  ...
  const [scenariosLoading, setScenariosLoading] = useState(scenarioId === "default");
  ...
  if (scenarioId === "default") {
    ...
    return (
      <div className="min-h-screen bg-slate-50" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
        ...
        <div className="bg-gradient-to-br from-slate-900 to-sky-950 px-6 py-10">
          <div className="max-w-2xl mx-auto">
            <button onClick={() => navigate('/')} className="flex items-center gap-1.5 text-sky-300 hover:text-white text-xs font-semibold mb-6 transition-colors">← Back to Dashboard</button>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-11 h-11 bg-sky-500 rounded-2xl flex items-center justify-center text-white font-black text-sm shadow-lg shadow-sky-900/40">14</div>
              <div>
                <span className="text-xs font-bold tracking-widest text-sky-400 uppercase">Stage 2 — Programs</span>
                <h1 style={{ fontFamily: "'DM Serif Display', Georgia, serif" }} className="text-2xl font-normal text-white mt-0.5">Conventional Refi Intelligence™</h1>
              </div>
            </div>
            <p className="text-sky-300 text-sm leading-relaxed mb-5">Analyze conventional refinance eligibility including RefiNow, RefiPossible, HIRO, and standard rate/term and cash-out options.</p>
            <div className="flex flex-wrap gap-2">
              {['RefiNow / RefiPossible', 'HIRO Eligible', 'Cash-Out Analysis', 'LTV Calculator', 'MI Removal', 'Decision Record'].map(tag => (
                <span key={tag} className="text-xs bg-white/10 border border-white/10 text-sky-200 px-3 py-1 rounded-full font-medium">{tag}</span>
              ))}
            </div>
          </div>
        </div>
        <div className="max-w-2xl mx-auto px-6 py-8">
          ...
          <div className="relative mb-4">
            ...
            <input type="text" value={search} onChange={e => { setSearch(e.target.value); setShowAll(false); }} placeholder="Search borrower name…"
              className="w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-sm text-slate-700 placeholder-slate-400 shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-300 focus:border-sky-300 transition-all" />
          ...
          {scenarios.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-3xl border border-slate-100 shadow-sm">
              ...
              <button onClick={() => navigate('/scenario-creator')} className="mt-4 text-xs font-bold text-sky-600 hover:text-sky-800 underline">→ Go to Scenario Creator</button>
            </div>
          ) : filtered.length === 0 ? (
            <div className="text-center py-10 bg-white rounded-3xl border border-slate-100 shadow-sm">
              ...
              <button onClick={() => setSearch('')} className="mt-2 text-xs text-sky-500 hover:underline">Clear search</button>
            </div>
          ) : (
            <div className="space-y-2.5">
              ...
              {displayed.map(s => {
                ...
                return (
                  <button key={s.id} onClick={() => navigate('/conventional-refi?scenarioId=' + s.id)}
                    className="w-full text-left bg-white border border-slate-200 rounded-2xl px-5 py-4 hover:border-sky-300 hover:shadow-md hover:bg-sky-50/30 transition-all group">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="font-semibold text-slate-800 text-sm truncate group-hover:text-sky-700">{sName}</div>
                        <div className="flex flex-wrap items-center gap-2 mt-1.5">
                          ...
                          {s.creditScore && <span className="text-xs bg-sky-50 text-sky-700 border border-sky-100 px-2 py-0.5 rounded-full font-mono">FICO {s.creditScore}</span>}
                        </div>
                      </div>
                      <span className="text-slate-300 group-hover:text-sky-400 text-lg shrink-0">→</span>
                    </div>
                  </button>
                );
              })}
              {hasMore && <button onClick={() => setShowAll(true)} className="w-full text-center text-xs font-bold text-sky-500 hover:text-sky-700 py-3 border border-dashed border-sky-200 rounded-2xl hover:bg-sky-50 transition-all">View all {filtered.length} scenarios</button>}
