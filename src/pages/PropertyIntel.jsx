...
// Collateral Intelligence™ — Module 21
    ...
    <div className="mt-4 border-t border-slate-200 pt-4 space-y-2">
      ...
      {fhaHardStop && (
        <div className="bg-red-100 border border-red-300 rounded-xl px-4 py-3 mt-2">
          ...
          {gain >= 100 && (<div className="text-xs text-red-600 mt-1 font-bold">⚠ Even after day 91, price increase of {gain}% triggers 2nd appraisal requirement until {fmtDate(fhaDay181)}</div>)}
        </div>
      )}
    </div>
  );
...
export default function CollateralIntel() {
  ...
  // ─── Decision Record
  const { reportFindings, savedRecordId, setSavedRecordId } = useDecisionRecord('PROPERTY_INTEL', scenarioId);
    ...
    getDoc(doc(db, 'scenarios', scenarioId)).then((snap) => {
      if (snap.exists()) {
        ...
        if (d.propertyType) {
          ...
          setPropertyType((prev) => prev || map[d.propertyType] || 'sfr');
        }
        if (d.occupancy) {
          const map = { 'Primary Residence': 'primary', 'Second Home': 'second', 'Investment Property': 'investment' };
          setOccupancy((prev) => prev || map[d.occupancy] || 'primary');
        }
  ...
  const { primarySuggestion, logFollow } = useNextStepIntelligence({
    currentModuleKey:       'PROPERTY_INTEL',
    ...
    decisionRecordFindings: {
      PROPERTY_INTEL: {
        ...
        appraisalRisk:    appraisalRisk || null,
        flipRisk:         appraisalConcerns?.flipRisk?.detected || false,
        appraisalReviewed: !!appraisalData,
      },
    },
    suggestions: [
      ...(overallVerdict === 'CRITICAL' || overallVerdict === 'FLAGS' ? [{
        moduleKey:           'AUS_RESCUE',
        moduleLabel:         'AUS Rescue™',
        route:               '/aus-rescue',
        urgency:             'HIGH',
        stage:               2,
        canSkip:             false,
        loanPurposeRelevant: true,
        reason:              `Collateral has ${criticalFlags.length > 0 ? criticalFlags.length + ' critical flag(s)' : highFlags.length + ' high flag(s)'}${appraisalRisk && (appraisalRisk === 'CRITICAL' || appraisalRisk === 'HIGH') ? ' and appraisal risk is ' + appraisalRisk : ''}. Return to AUS Rescue™ to evaluate program eligibility impact before proceeding.`,
      }] : []),
      {
        moduleKey:           'TITLE_INTEL',
        moduleLabel:         'Title Intelligence™',
        route:               '/title-intel',
        urgency:             flaggedItems.length > 0 ? 'HIGH' : 'MEDIUM',
        stage:               3,
        canSkip:             false,
        loanPurposeRelevant: true,
        reason:              overallVerdict === 'CLEAR'
          ? 'Collateral review clear. Proceed to Title Intelligence™ for lien and vesting review before disclosures.'
          : `Collateral flagged ${flaggedItems.length} condition issue(s). Document in Title Intelligence™ and confirm title is clear before proceeding to disclosures.`,
      },
      {
        moduleKey:           'FLOOD_INTEL',
        moduleLabel:         'Flood Intelligence™',
        route:               '/flood-intel',
        urgency:             flags.flood_zone ? 'HIGH' : 'MEDIUM',
        stage:               3,
        canSkip:             true,
        loanPurposeRelevant: true,
        reason:              flags.flood_zone
          ? 'Property is flagged in a FEMA flood zone. Run Flood Intelligence™ to confirm NFIP requirements and coverage minimums.'
          : 'Run Flood Intelligence™ to confirm flood zone determination and insurance requirements.',
      },
    ],
  });

  // ─── Appraisal upload handler
  ...
  const handleSaveToRecord = async () => {
    ...
    try {
      const riskFlags = [];
      criticalFlags.forEach((f) => riskFlags.push({ flagCode: f.id.toUpperCase(), sourceModule: 'COLLATERAL_INTEL', severity: 'HIGH',   detail: f.label }));
      highFlags.forEach((f)     => riskFlags.push({ flagCode: f.id.toUpperCase(), sourceModule: 'COLLATERAL_INTEL', severity: 'MEDIUM', detail: f.label }));
      if (appraisalConcerns?.overallRisk === 'CRITICAL' || appraisalConcerns?.overallRisk === 'HIGH')
        riskFlags.push({ flagCode: 'APPRAISAL_REVIEW_RISK', sourceModule: 'COLLATERAL_INTEL', severity: appraisalConcerns.overallRisk === 'CRITICAL' ? 'HIGH' : 'MEDIUM', detail: 'Appraisal review: ' + appraisalConcerns.overallRisk + ' risk — ' + (appraisalConcerns.concerns?.length || 0) + ' concerns' });
      if (appraisalConcerns?.flipRisk?.detected)
        riskFlags.push({ flagCode: 'FLIP_RISK_DETECTED', sourceModule: 'COLLATERAL_INTEL', severity: 'HIGH', detail: 'Flip risk: ' + (appraisalConcerns.flipRisk.summary || '') });
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <h1 style={{ fontFamily: "'DM Serif Display', Georgia, serif" }} className="text-2xl font-normal text-white mt-0.5">Collateral Intelligence™</h1>
              </div>
            </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <DecisionRecordBanner savedRecordId={savedRecordId} moduleKey="PROPERTY_INTEL" />
      ...
      <div className="bg-slate-900 relative overflow-hidden" style={{ minHeight: '200px' }}>
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="flex items-start justify-between flex-wrap gap-6">
            <div>
              <span className="text-xs font-bold tracking-widest text-indigo-400 uppercase">Stage 3 — Property &amp; Closing</span>
              <h1 style={{ fontFamily: "'DM Serif Display', Georgia, serif" }} className="text-4xl font-normal text-white mb-2 mt-0.5">Collateral Intelligence™</h1>
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                {['Appraisal Review', 'Stale Comp Detection', 'Flip Risk', 'FHA/VA Flags', 'Waiver Coach'].map(tag => (
                  <span key={tag} className="text-xs bg-white/10 border border-white/10 text-indigo-200 px-3 py-1 rounded-full font-medium">{tag}</span>
                ))}
              </div>
            </div>
            <div className="bg-slate-800/60 border border-slate-700 rounded-2xl px-5 py-4" style={{ minWidth: '240px', flexShrink: 0 }}>
              {scenario ? (
                <>
                  ...
                  <button onClick={() => navigate('/property-intel')} className="text-xs text-blue-400 hover:text-blue-300 mt-2 block transition-colors">Change scenario →</button>
                </>
              ) : (
      ...
      {/* 4 — Scenario Header */}
      <ScenarioHeader moduleTitle="Collateral Intelligence™" moduleNumber="21" scenarioId={scenarioId} />
            ...
            {TABS.map((tab) => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                className={'flex items-center gap-2 px-6 py-4 text-sm font-semibold border-b-2 transition-all ' +
                  (activeTab === tab.id ? 'border-emerald-500 text-emerald-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300')}>
              ...
              <>
                      ...
                      <div className={'mt-4 rounded-2xl border px-5 py-4 ' + (propType.eligible.length === 0 ? 'bg-red-50 border-red-200' : 'bg-emerald-50 border-emerald-200')}>
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                          {['FHA','VA','USDA','Conventional','Jumbo'].map((p) => (
                            <span key={p} className={'text-xs px-2 py-1 rounded-lg font-bold ' + (propType.eligible.includes(p) ? 'bg-emerald-200 text-emerald-800' : 'bg-red-100 text-red-600 line-through')}>{p}</span>
                          ))}
                        </div>
                      </div>
                    )}
                ...
                <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                  <div className="bg-gradient-to-r from-slate-800 to-slate-700 px-8 py-5">
                    ...
                    <p className="text-slate-400 text-sm mt-1">Auto-populated from scenario. Year Built and Sq Ft auto-fill from uploaded appraisal PDF.</p>
                  </div>
                  <div className="p-8">
                ...
                <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                  <div className="bg-gradient-to-r from-slate-800 to-slate-700 px-8 py-5">
                    ...
                    <p className="text-slate-400 text-sm mt-1">Check all conditions that apply. Flagged items are logged to the Decision Record.</p>
                  </div>
                  <div className="p-8 space-y-3">
                    {PROPERTY_FLAGS.map((flag) => (
                      <label key={flag.id} className={'flex items-start gap-4 p-4 rounded-2xl border-2 cursor-pointer transition-all ' +
                        (flags[flag.id] ? (flag.severity === 'critical' ? 'bg-red-50 border-red-300' : flag.severity === 'high' ? 'bg-orange-50 border-orange-300' : 'bg-amber-50 border-amber-200') : 'bg-slate-50 border-slate-200 hover:border-slate-300')}>
                ...
                {/* ── Next Step Intelligence™ — shown on Tab 0 ── */}
                {primarySuggestion && (
              ...
              <>
                  ...
                  <div className="p-8 space-y-6">
                    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4">
                      <p className="text-xs text-slate-600 leading-relaxed">
                        <strong>How it works:</strong> Step 1 — Haiku extracts subject data, appraised value, comparables (with <strong>contract dates</strong>), prior sale history, and conditions. Step 2 — Sonnet analyzes against {loanType || 'agency'} guidelines, counts stale comps from effective date, detects flip risk. Step 3 — Generate rebuttal letter for rebuttable concerns.
                      </p>
                    </div>
                    <div>
                      <label className={'flex flex-col items-center justify-center w-full h-36 border-2 border-dashed rounded-2xl cursor-pointer transition-all ' + (appraisalExtracting ? 'border-emerald-400 bg-emerald-50' : 'border-slate-300 hover:border-emerald-400 hover:bg-emerald-50')}>
                        ...
                        {appraisalExtracting ? (
                          <div className="flex flex-col items-center gap-3">
                            <div className="w-8 h-8 border-emerald-400 border-t-transparent rounded-full animate-spin" style={{ borderWidth: '3px', borderStyle: 'solid' }} />
                          ...
                          <div className="flex flex-col items-center gap-2">
                            ...
                            <div className="text-sm font-bold text-slate-700">{appraisalData ? 'Re-upload Appraisal PDF' : 'Upload Appraisal PDF'}</div>
                          ...
                          <button onClick={() => setShowRebuttal((v) => !v)} className="w-full flex items-center justify-between px-6 py-4 bg-blue-50 hover:bg-blue-100 transition-colors">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="text-left">
                                ...
                                <div className="text-xs text-blue-600">{appraisalConcerns.concerns.filter((c) => c.rebuttable).length} rebuttable concern(s) — letter auto-generated</div>
                              </div>
                            </div>
                ...
                {!appraisalData && !appraisalExtracting && !appraisalError && (
                  <div className="bg-slate-50 border-2 border-dashed border-slate-300 rounded-3xl p-12 text-center">
                    ...
                    <p className="text-slate-400 text-sm max-w-md mx-auto">Upload the appraisal PDF above to run the AI analysis.</p>
                  </div>
                )}
              </>
            )}
              ...
              <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div className="p-8 space-y-6">
                  <div className="bg-indigo-50 border border-indigo-200 rounded-2xl px-5 py-4">
                    <p className="text-sm text-indigo-800 leading-relaxed"><strong>What is an appraisal waiver?</strong> GSEs (Fannie Mae and Freddie Mac) and the VA allow certain loans to skip the full appraisal using automated data models. A waiver can save the borrower $500-$900, eliminate 7-14 days from the timeline, and remove the appraisal contingency — but it must be offered by the AUS (DU or LP) and the property must meet eligibility criteria.</p>
                  </div>
                  <div className="grid grid-cols-3 gap-4">
                    {WAIVER_TYPES.map((wt) => (
                    ...
                    <div key={wt.id} className="space-y-5">
                      ...
                      <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5">
                        ...
                        <div className="space-y-2">
                          {wt.criteria.map((c, i) => (
                            <div key={i} className="flex items-start gap-2 text-sm"><span className="text-indigo-400 shrink-0 mt-0.5">•</span><span className="text-slate-700">{c}</span></div>
                          ))}
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4">
                          ...
                          {wt.benefits.map((b, i) => <div key={i} className="flex gap-2 text-xs text-emerald-800 mb-2"><span className="shrink-0 text-emerald-500">✓</span><span>{b}</span></div>)}
                        </div>
                        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
                          <div className="text-xs font-bold text-amber-700 uppercase tracking-wide mb-3">Risks to Know</div>
                          {wt.risks.map((r, i) => <div key={i} className="flex gap-2 text-xs text-amber-800 mb-2"><span className="shrink-0 text-amber-500">⚠</span><span>{r}</span></div>)}
                        </div>
                      </div>
            ...
            {activeTab === 3 && (
              <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="bg-gradient-to-r from-slate-800 to-slate-700 px-8 py-5">
                  ...
                  <p className="text-slate-400 text-sm mt-1">Document property observations, appraisal concerns, repair requirements, and special conditions.</p>
                </div>
                <div className="p-8 space-y-6">
              ...
              <div className={'rounded-2xl p-4 text-center mb-5 ' + (overallVerdict === 'CLEAR' ? 'bg-emerald-900/30 border border-emerald-700/40' : overallVerdict === 'CRITICAL' ? 'bg-red-900/30 border border-red-700/40' : 'bg-amber-900/30 border border-amber-700/40')}>
                ...
                <div className="text-xs text-slate-400 mt-1">
                  {overallVerdict === 'CLEAR'
                    ? 'No condition issues detected'
                    : appraisalRisk && (appraisalRisk === 'CRITICAL' || appraisalRisk === 'HIGH') && criticalFlags.length === 0
                      ? 'Appraisal risk: ' + appraisalRisk
                      : criticalFlags.length + ' critical, ' + highFlags.length + ' high flag(s)'}
                </div>
              </div>
              <div className="space-y-2">
                {[
              ...
              {appraisalData && (
                <div className={'mt-4 rounded-2xl border p-4 ' + (appraisalConcerns?.overallRisk === 'CRITICAL' || appraisalConcerns?.overallRisk === 'HIGH' ? 'bg-red-900/20 border-red-700/40' : 'bg-indigo-900/20 border-indigo-700/40')}>
                  ...
                  <div className="text-xs text-slate-400">
                    {appraisalConcerns?.concerns?.length || 0} concern(s) · {appraisalConcerns?.overallRisk || 'not analyzed'}
                    {appraisalConcerns?.staleCompCount > 0 && ' · ' + appraisalConcerns.staleCompCount + ' stale'}
                    {appraisalConcerns?.flipRisk?.detected && ' · ⚠ Flip risk'}
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-3xl p-5">
              ...
              <ul className="space-y-2">
                {[
                  ...
                  'FHA flip rule: hard stop < 91 days · 2nd appraisal 91-180 days if gain ≥100%',
                  'USDA: follows FHA flip seasoning rules',
                  'VA/Conventional: no hard flip rule — underwriter scrutiny if < 12 months',
                  'Stale comps: use CONTRACT date (c/), not settlement date (s/)',
                  'Well/septic: water potability test required for FHA/VA',
