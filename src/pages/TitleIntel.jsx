...
// Title Intelligence™ v3.0 — Upload-First Architecture
...
JSON schema:
{
  "vestingFound": "exact id from: ${validVestingIds} — or null",
...
STOP: any critical issue making closing impossible today. HIGH_RISK: 2+ HIGH items, vesting/loan type conflict, tight timeline. CAUTION: medium items, unconfirmed payoffs, vesting unconfirmed. CLEAR: no issues, confirmed, report received. One issueResolution per flagged issue — never skip.`;
  ...
  if (loading) return <div className="min-h-screen bg-slate-50 flex items-center justify-center"><div className="animate-spin w-6 h-6 border-2 border-indigo-400 border-t-transparent rounded-full" /></div>;
      ...
      <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-2xl mx-auto px-6 py-8">
          ...
          <div className="relative mb-4">
            ...
            <input value={search} onChange={e => { setSearch(e.target.value); setShowAll(false); }} placeholder="Search borrower name…" className="w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-2xl text-sm placeholder-slate-400 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 transition-all" />
          ...
          {scenarios.length === 0 ? (
            ...
            <div className="space-y-2.5">
              {displayed.map(s => {
                ...
                return (
                  <button key={s.id} onClick={() => navigate(`/title-intel?scenarioId=${s.id}`)} className="w-full text-left bg-white border border-slate-200 rounded-2xl px-5 py-4 hover:border-indigo-300 hover:shadow-md hover:bg-indigo-50/30 transition-all group">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-indigo-200 text-sm mt-0.5">{borrowerName ? `${borrowerName} · ` : ''}Vesting · Liens · Chain of Title · AI Risk Analysis</p>
            </div>
            <div className="text-right">
              {riskCfg ? (
                <div className={`${riskCfg.heroBg} ${riskCfg.heroBorder} border rounded-2xl px-4 py-2 text-center`}>
                  <div className={`text-lg font-black ${riskCfg.heroText}`}>{riskCfg.emoji}</div>
                  <div className={`text-xs font-bold ${riskCfg.heroText}`}>{riskCfg.label}</div>
                </div>
              ) : criticalIssues.length > 0 ? (
                <div className="bg-red-500/20 border border-red-400/30 rounded-2xl px-4 py-2 text-center">
                  <div className="text-2xl font-black text-red-300">{criticalIssues.length}</div>
                ...
                <div className="bg-white/10 border border-white/10 rounded-2xl px-4 py-2 text-center">
                  <div className="text-2xl font-black text-white">{flaggedIssues.length > 0 ? flaggedIssues.length : '✓'}</div>
          ...
          <div className="flex gap-1 mt-5 bg-white/10 rounded-2xl p-1 w-fit">
            {TABS.map((tab, i) => (
              <button key={tab} onClick={() => setActiveTab(i)} className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all whitespace-nowrap ${activeTab === i ? 'bg-white text-slate-800 shadow-sm' : 'text-indigo-200 hover:text-white'}`}>
                {i === 0 && !extractionApplied && <span className="mr-1.5 w-2 h-2 bg-indigo-400 rounded-full inline-block" />}
                {tab}
                {i === 2 && aiAnalysis && <span className={`ml-1.5 text-xs px-1.5 py-0.5 rounded-full ${RISK_CONFIG[aiAnalysis.riskRating]?.badge}`}>{aiAnalysis.riskRating}</span>}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
          <div className="xl:col-span-2 space-y-5">
                          ...
                          {prelimExtraction.flaggedIssueIds?.length > 0 && (
                            <div className="bg-white border border-slate-200 rounded-2xl p-4">
                              ...
                              <div className="space-y-1.5">
                                {prelimExtraction.flaggedIssueIds.map(id => { const issue = TITLE_ISSUES.find(i => i.id === id); return issue ? (
            ...
            {activeTab === 1 && (
              <>
                {loanTypeFlags.length > 0 && <div className="space-y-2">{loanTypeFlags.map((f,i) => (<div key={i} className={`rounded-3xl border px-5 py-4 flex items-start gap-3 ${f.severity==='critical'?'bg-red-50 border-red-300':'bg-orange-50 border-orange-200'}`}><span className="text-base shrink-0">{f.severity==='critical'?'🛑':'⚠️'}</span><p className={`text-sm font-semibold ${f.severity==='critical'?'text-red-700':'text-orange-700'}`}>{f.msg}</p></div>))}</div>}
                {timelineFlags.length > 0 && <div className="bg-orange-50 border border-orange-200 rounded-3xl p-4 space-y-1.5">{timelineFlags.map((f,i) => <p key={i} className="text-xs text-orange-700 font-semibold">{f}</p>)}</div>}
                <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
                ...
                <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <select value={titleCompany} onChange={e => setTitleCompany(e.target.value)} className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-indigo-300 focus:outline-none"><option value="">Select…</option>{TITLE_COMPANIES.map(t => <option key={t} value={t}>{t}</option>)}</select>
                  ...
                  <div className="flex gap-3">
                    {[['titleOrdered','📋 Title Search Ordered',titleOrdered,setTitleOrdered],['titleReceived','📄 Prelim Report Received',titleReceived,setTitleReceived]].map(([id,label,val,setter]) => (
                      <label key={id} className={`flex items-center gap-2 px-4 py-3 rounded-2xl border cursor-pointer transition-all flex-1 justify-center ${val?'bg-emerald-50 border-emerald-300':'bg-slate-50 border-slate-200'}`}><input type="checkbox" checked={val} onChange={e => setter(e.target.checked)} className="accent-emerald-600" /><span className={`text-sm font-semibold ${val?'text-emerald-700':'text-slate-600'}`}>{label}</span></label>
                    ))}
                  </div>
                </div>
                <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
                  <div className="flex items-center justify-between mb-1"><h2 className="text-sm font-bold text-slate-500 uppercase tracking-wide">💳 Liens to Pay Off at Closing</h2><button onClick={addLien} className="text-xs font-bold text-indigo-600 bg-indigo-50 border border-indigo-200 px-3 py-1.5 rounded-xl hover:bg-indigo-100 transition-colors">+ Add Lien</button></div>
                  ...
                  {liens.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {liens.map(l => (
                        <div key={l.id} className={`rounded-2xl border p-4 ${l.payoffConfirmed?'bg-emerald-50 border-emerald-200':'bg-amber-50 border-amber-200'}`}>
                          <div className="flex items-center gap-2 flex-wrap">
                            <select value={l.type} onChange={e => updateLien(l.id,'type',e.target.value)} className="border border-slate-200 rounded-xl px-2.5 py-1.5 text-xs bg-white focus:outline-none"><option value="">Type…</option>{['1st Mortgage','2nd Mortgage','HELOC','Tax Lien','Judgment','HOA','Mechanic\'s Lien','Other'].map(t => <option key={t}>{t}</option>)}</select>
                            ...
                            <label className="flex items-center gap-1.5 text-xs cursor-pointer whitespace-nowrap"><input type="checkbox" checked={l.payoffConfirmed} onChange={e => updateLien(l.id,'payoffConfirmed',e.target.checked)} className="accent-emerald-600" /><span className={l.payoffConfirmed?'text-emerald-700 font-bold':'text-amber-700 font-semibold'}>{l.payoffConfirmed?'✓ Confirmed':'Confirm'}</span></label>
                      ...
                      <div className="flex justify-between px-4 py-3 bg-slate-50 rounded-2xl font-bold text-sm border border-slate-200"><span className="text-slate-500">Total Payoffs</span><span className="text-slate-800">{fmt$(totalLienAmount)}</span></div>
                    </div>
                  )}
                </div>
                <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
                ...
                <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-2xl p-3.5 text-xs text-blue-700 space-y-1"><p>• Lender's title insurance is mandatory on all agency loans</p><p>• Owner's policy is optional but strongly recommended</p><p>• Simultaneous issue discount available when purchasing both</p></div>
                </div>
                <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
                  <h2 className="text-sm font-bold text-slate-500 uppercase tracking-wide mb-3">📝 LO Notes</h2>
                  <textarea value={loNotes} onChange={e => setLoNotes(e.target.value)} rows={5} placeholder="Title search findings, lien resolution status, attorney recommendations, timeline concerns…" className="w-full border border-slate-200 rounded-2xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-300 focus:outline-none resize-none" />
                </div>
              ...
              <div className="space-y-5">
                {!aiAnalysis && !aiLoading && (
                  <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
                    ...
                    <div className="space-y-2 mb-5">{readinessChecks.map(({ label, done }) => (<div key={label} className="flex items-center gap-2.5"><span className={`text-sm shrink-0 ${done?'text-emerald-500':'text-slate-300'}`}>{done?'✓':'○'}</span><span className={`text-sm ${done?'text-slate-700 font-medium':'text-slate-400'}`}>{label}</span></div>))}</div>
                    <div className="text-center pt-2"><div className="text-4xl mb-3">🔍</div><h3 className="text-lg font-bold text-slate-800 mb-2" style={{ fontFamily: '"DM Serif Display", serif' }}>AI Title Risk Analysis</h3><p className="text-sm text-slate-500 mb-5 max-w-md mx-auto">Claude analyzes your full title profile — vesting, liens, every flagged issue — and generates a risk rating, issue resolutions, and both letters.</p><button onClick={runAiAnalysis} className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold px-8 py-3 rounded-2xl transition-colors shadow-md shadow-indigo-200">Run Title Risk Analysis</button></div>
                  </div>
                )}
                {aiLoading && (<div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-10 text-center"><div className="animate-spin w-8 h-8 border-2 border-indigo-500 border-t-transparent rounded-full mx-auto mb-4" /><p className="text-sm font-semibold text-slate-600">Analyzing title profile…</p><p className="text-xs text-slate-400 mt-1">Generating risk rating, resolutions, and letters</p></div>)}
                {aiError && !aiLoading && (<div className="bg-red-50 border border-red-200 rounded-3xl p-5"><p className="text-sm font-bold text-red-700 mb-1">Analysis Error</p><p className="text-xs text-red-600">{aiError}</p><button onClick={runAiAnalysis} className="mt-3 text-xs font-bold text-red-600 underline">Try again</button></div>)}
                {aiAnalysis && !aiLoading && (() => {
                    ...
                    <>
                      <div className={`${cfg.bg} ${cfg.border} border rounded-3xl p-5`}>
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2 flex-wrap"><span className="text-2xl">{cfg.emoji}</span><span className={`text-xl font-black ${cfg.text}`} style={{ fontFamily: '"DM Serif Display", serif' }}>{cfg.label}</span><span className={`text-xs font-bold px-2 py-1 rounded-full ${cfg.badge}`}>{aiAnalysis.closingReadiness?.replace('_',' ')}</span></div>
                          ...
                          <button onClick={runAiAnalysis} className="text-xs font-bold text-slate-500 hover:text-slate-700 bg-white border border-slate-200 px-3 py-1.5 rounded-xl shrink-0">↻ Re-analyze</button>
                        </div>
                      </div>
                      {aiAnalysis.recommendedConditions?.length > 0 && (
                        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-5">
                          ...
                          <div className="space-y-2">{aiAnalysis.recommendedConditions.map((c,i) => (<div key={i} className="flex items-start gap-2 bg-slate-50 rounded-2xl px-3 py-2.5"><span className="text-slate-400 text-xs font-bold shrink-0 mt-0.5">{i+1}.</span><p className="text-xs text-slate-700">{c}</p></div>))}</div>
                        </div>
                      )}
                      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
                        ...
                        <div className="p-6">
                          {aiTab === 0 && (
                            <div className="space-y-5">
                              ...
                              {aiAnalysis.issueResolutions?.length > 0 ? (
                                <div><h4 className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-3">🚩 Issue-by-Issue Resolution Guidance</h4>
                                  <div className="space-y-3">
                                    {aiAnalysis.issueResolutions.map((item,idx) => { const sev=item.severity; const colors=sev==='critical'?'bg-red-50 border-red-200 text-red-800':sev==='high'?'bg-orange-50 border-orange-200 text-orange-800':sev==='info'?'bg-blue-50 border-blue-200 text-blue-800':'bg-amber-50 border-amber-200 text-amber-800'; const badge=sev==='critical'?'bg-red-100 text-red-700':sev==='high'?'bg-orange-100 text-orange-700':sev==='info'?'bg-blue-100 text-blue-700':'bg-amber-100 text-amber-700'; return (
                                      <div key={idx} className={`rounded-2xl border p-4 ${colors}`}>
                                        <div className="flex items-center gap-2 flex-wrap mb-2"><span className="text-sm font-bold">{item.issueLabel}</span><span className={`text-xs px-2 py-0.5 rounded-full font-bold ${badge}`}>{sev.toUpperCase()}</span>{item.estimatedDays && <span className="text-xs bg-white/60 px-2 py-0.5 rounded-full font-semibold opacity-80">⏱ {item.estimatedDays}</span>}</div>
                          ...
                          {aiTab === 1 && (
                            <div>
                              <div className="flex items-center justify-between mb-4"><div><h4 className="text-xs font-bold text-slate-500 uppercase tracking-wide">Borrower Explanation Letter</h4><p className="text-xs text-slate-400 mt-0.5">Plain-language · Ready to send</p></div><button onClick={() => navigator.clipboard.writeText(aiAnalysis.borrowerLetter||'')} className="text-xs font-bold text-indigo-600 bg-indigo-50 border border-indigo-200 px-3 py-1.5 rounded-xl hover:bg-indigo-100">Copy</button></div>
                          ...
                          {aiTab === 2 && (
                            <div>
                              <div className="flex items-center justify-between mb-4"><div><h4 className="text-xs font-bold text-slate-500 uppercase tracking-wide">Underwriter Summary Memo</h4><p className="text-xs text-slate-400 mt-0.5">Professional · Formal tone</p></div><button onClick={() => navigator.clipboard.writeText(aiAnalysis.underwriterSummary||'')} className="text-xs font-bold text-indigo-600 bg-indigo-50 border border-indigo-200 px-3 py-1.5 rounded-xl hover:bg-indigo-100">Copy</button></div>
          ...
          <div className="xl:col-span-1 space-y-4 xl:sticky xl:top-6 self-start">
            <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-5">
              ...
              <div className="space-y-2.5 text-xs">
                {[
                  ...
                  <div key={label} className="flex justify-between gap-2"><span className="text-slate-400 shrink-0">{label}</span><span className="font-semibold text-slate-700 text-right max-w-[55%] truncate">{value}</span></div>
                ))}
              </div>
              {riskCfg && <div className="mt-4 pt-3 border-t border-slate-100"><div className={`${riskCfg.bg} ${riskCfg.border} border rounded-2xl px-3 py-2.5 flex items-center gap-2`}><span className="text-base">{riskCfg.emoji}</span><div><div className={`text-xs font-black ${riskCfg.text}`}>AI Risk: {riskCfg.label}</div><div className={`text-xs ${riskCfg.text} opacity-70`}>{aiAnalysis?.closingReadiness?.replace('_',' ')}</div></div></div></div>}
            </div>
            {flaggedIssues.length > 0 && (
              <div className={`rounded-3xl border p-4 ${criticalIssues.length>0?'bg-red-50 border-red-200':'bg-amber-50 border-amber-200'}`}>
                ...
                <div className="space-y-1.5">{flaggedIssues.map(i => (<div key={i.id} className="text-xs"><span className={i.severity==='critical'?'text-red-600 font-bold':i.severity==='high'?'text-orange-700 font-semibold':'text-amber-700'}>{i.severity==='critical'?'🛑':i.severity==='high'?'🔴':'⚠️'} {i.label}</span></div>))}</div>
              </div>
            )}
            ...
            <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-4">
              ...
              <div className="space-y-1.5">{TABS.map((tab,i) => (<button key={tab} onClick={() => setActiveTab(i)} className={`w-full text-left text-xs px-3 py-2 rounded-xl transition-colors font-medium ${activeTab===i?'bg-indigo-50 text-indigo-700 font-bold':'text-slate-500 hover:bg-slate-50'}`}>{i+1}. {tab}</button>))}</div>
            </div>
          </div>
        </div>
      </div>
