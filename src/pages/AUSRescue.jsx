...
// ── PROGRAM MIGRATION ENGINE (Layer 2) ───────────────────────────────────────
// Ranks alternative programs by approval probability given borrower profile
const MIGRATION_PROGRAMS = [
  { key:'fha',          label:'FHA 3.5%',         minFICO:580, maxDTI:56.9, minDown:3.5, note:'FHA TOTAL Scorecard — most flexible DTI' },
  { key:'conventional', label:'Conventional 3%',  minFICO:620, maxDTI:50,   minDown:3,   note:'DU/LPA — best pricing 740+ FICO' },
  { key:'homeready',    label:'HomeReady 3%',      minFICO:620, maxDTI:50,   minDown:3,   note:'FNMA — reduced MI, income ≤80% AMI' },
  { key:'homepossible', label:'Home Possible 3%',  minFICO:660, maxDTI:45,   minDown:3,   note:'FHLMC — reduced MI, income ≤80% AMI' },
  { key:'va',           label:'VA 0%',             minFICO:580, maxDTI:60,   minDown:0,   note:'Veteran only — residual income primary' },
  { key:'usda',         label:'USDA 0%',           minFICO:640, maxDTI:41,   minDown:0,   note:'Rural + income eligible — dual DTI 29/41' },
  { key:'fha203k',      label:'FHA 203(k)',         minFICO:580, maxDTI:57,   minDown:3.5, note:'Renovation — rehab ≤$35K streamlined' },
  { key:'homestyle',    label:'HomeStyle Reno',     minFICO:620, maxDTI:50,   minDown:3,   note:'Fannie — any reno amount' },
  { key:'bank_stmt',    label:'Bank Statement NonQM',minFICO:600,maxDTI:50,  minDown:10,  note:'Self-employed — 12/24mo deposits' },
  { key:'dscr',         label:'DSCR NonQM',         minFICO:620, maxDTI:null, minDown:20,  note:'Investment — cash flow qualifying' },
  { key:'asset_dep',    label:'Asset Depletion NonQM',minFICO:620,maxDTI:50, minDown:20,  note:'Retirees — assets ÷ 360 = income' },
];

const scoreMigrationProgram = (prog, profile) => {
  const fico = +profile.creditScore || 0;
  ...
  // FICO
  if (fico < prog.minFICO) { score -= 40; blockers.push(`FICO ${fico} below minimum ${prog.minFICO}`); }
  else if (fico >= prog.minFICO + 60) { score += 5; strengths.push(`Strong FICO ${fico}`); }

  // DTI
  if (prog.maxDTI && dti > prog.maxDTI) { score -= 35; blockers.push(`DTI ${dti}% exceeds ${prog.maxDTI}% max`); }
  else if (prog.maxDTI && dti <= prog.maxDTI * 0.85) { score += 5; strengths.push(`DTI ${dti}% well within ${prog.maxDTI}%`); }

  // Down payment
  if (down < prog.minDown) { score -= 25; blockers.push(`Down ${down}% below ${prog.minDown}% minimum`); }

  // FICO sweet-spot adjustments
  if (prog.key === 'fha' && fico >= 680) { score -= 8; } // FHA penalizes high FICO vs conv
  if ((prog.key === 'homeready' || prog.key === 'homepossible') && fico >= 720) { score += 8; strengths.push('Strong FICO pricing tier'); }
  if (prog.key === 'va' && !profile.isVeteran) { score -= 60; blockers.push('VA eligibility required'); }
  if (prog.key === 'usda' && !profile.isRuralProperty) { score -= 50; blockers.push('Rural property required'); }
  if ((prog.key === 'fha203k' || prog.key === 'homestyle') && !profile.isRehabProperty) { score -= 30; blockers.push('Renovation property required'); }
  if (prog.key === 'dscr' && !profile.isInvestmentProperty) { score -= 40; blockers.push('Investment property required'); }
  if ((prog.key === 'bank_stmt' || prog.key === 'asset_dep') && !profile.isSelfEmployed && !profile.isHighAssetBorrower) { score -= 20; }

  return { ...prog, score: Math.max(0, Math.min(100, score)), blockers, strengths };
};
...
export default function AUSRescue() {
  ...
  useEffect(() => {
    ...
    setProfile(prev => ({
      ...
      isVeteran:    scenarioDoc.vaEntitlement === 'FULL' || scenarioDoc.loanType === 'VA',
    }));
  ...
  // ── Migration Engine rankings ─────────────────────────────────────────────
  const migrationRankings = useMemo(() => {
    if (!ruleEngineInput) return [];
    return MIGRATION_PROGRAMS.map(p => scoreMigrationProgram(p, { ...ruleEngineInput, ...profile }))
      .sort((a, b) => b.score - a.score);
  }, [ruleEngineInput, profile]);
  ...
  const handleSaveToRecord = async () => {
    ...
    try {
      ...
      const writtenRecordId = await reportFindings('AUS_RESCUE', {
        ...
        migrationRankings: migrationRankings.slice(0, 5).map(p => ({ key: p.key, label: p.label, score: p.score })),
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
          ...
          <div className="xl:col-span-1 space-y-5">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
              ...
              {/* AUS Finding Input */}
              <div>
                <p className="text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">AUS Finding for {PROGRAMS[program]?.label}</p>
          ...
          <div className="xl:col-span-3 space-y-5">
            ...
            <div className="flex flex-wrap gap-2 border-b border-gray-200 pb-0">
              {[{k:'strategies',l:'🎯 Strategies'},{k:'programs',l:'📊 Program Comparison'},{k:'migration',l:'🔄 Migration Engine'},{k:'duplicates',l:'🔍 Duplicate Debt Detector'}].map(t => (
                ...
                {scoredPrograms.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {scoredPrograms.map((prog, idx) => {
                      ...
                      const fs = FEAS_STYLE[prog.likelihood === 'High' ? 'HIGH' : prog.likelihood === 'Medium' ? 'MEDIUM' : 'LOW'];
                      const isTop = idx === 0;
                      return (
            ...
            {/* ── MIGRATION ENGINE TAB ───────────────────────────────── */}
            {activeTab === 'migration' && (
              <div>
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                  <div className="px-5 py-4 border-b border-gray-100">
                    <h3 className="text-sm font-bold text-gray-800">Program Migration Engine</h3>
                    <p className="text-xs text-gray-500 mt-0.5">All 11 programs ranked by approval probability based on borrower profile. Enter Credit Score and DTI to activate.</p>
                  </div>
                  {migrationRankings.length === 0 ? (
                    <div className="p-10 text-center text-gray-400 text-sm">Enter Credit Score and DTI in Step 1 to run the Migration Engine.</div>
                  ) : (
                    <div className="divide-y divide-gray-100">
                      {migrationRankings.map((prog, idx) => (
                        <div key={prog.key} className={`flex items-center gap-4 px-5 py-3 hover:bg-gray-50 transition-colors ${idx === 0 ? 'bg-emerald-50/50' : ''}`}>
                          <span className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0 ${idx === 0 ? 'bg-emerald-500 text-white' : idx <= 2 ? 'bg-indigo-100 text-indigo-700' : 'bg-gray-100 text-gray-500'}`}>{idx + 1}</span>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <p className="text-sm font-bold text-gray-800">{prog.label}</p>
                              {prog.blockers?.length === 0 && <span className="text-[10px] bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded font-semibold">ELIGIBLE</span>}
                            </div>
                            <p className="text-xs text-gray-400">{prog.note}</p>
                            {prog.blockers?.length > 0 && (
                              <div className="flex flex-wrap gap-1 mt-1">
                                {prog.blockers.map((b, i) => <span key={i} className="text-[10px] text-red-600 bg-red-50 border border-red-200 px-1.5 py-0.5 rounded">✗ {b}</span>)}
                              </div>
                            )}
                            {prog.strengths?.length > 0 && (
                              <div className="flex flex-wrap gap-1 mt-1">
                                {prog.strengths.slice(0, 2).map((s, i) => <span key={i} className="text-[10px] text-emerald-700 bg-emerald-50 border border-emerald-200 px-1.5 py-0.5 rounded">✓ {s}</span>)}
                              </div>
                            )}
                          </div>
                          <div className="flex-shrink-0 text-right">
                            <div className={`text-xl font-black ${pColor(prog.score)}`}>{prog.score}%</div>
                            <div className="w-20 h-1.5 bg-gray-100 rounded-full mt-1">
                              <div className={`h-full rounded-full ${pBar(prog.score)}`} style={{ width: `${prog.score}%` }} />
                            </div>
                          </div>
                        </div>
                      ))}
