    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-3xl mx-auto px-4">
        ...
        <div className="flex items-center gap-3 mb-2">
          <div className="w-10 h-10 bg-gradient-to-br from-violet-600 to-indigo-600 rounded-xl flex items-center justify-center text-white font-black text-sm">06</div>
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Debt Resolution Engine™</h1>
            <p className="text-sm text-gray-500">Stage 1 — Pre-Structure & Initial Analysis</p>
          </div>
        </div>
        <div className="bg-violet-50 border border-violet-200 rounded-xl p-4 mb-6 mt-4">
          <p className="text-sm text-violet-800 font-semibold">Find the fastest, cheapest path to qualification.</p>
          <p className="text-xs text-violet-600 mt-1">Upload the credit report → AI extracts all debts → Engine builds the minimum-cost Pay-to-Close plan → Export borrower-ready PDF.</p>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <h2 className="font-bold text-gray-800 mb-4">Select a Scenario</h2>
          {scenarios.length === 0
            ? <p className="text-gray-400 text-sm">No scenarios found. Create one first.</p>
            : <div className="space-y-3">
                {scenarios.map(s => (
                  <button key={s.id} onClick={() => selectScenario(s)}
                    className="w-full text-left p-4 border border-gray-200 rounded-xl hover:border-violet-400 hover:bg-violet-50 transition-all">
                    <div className="font-semibold text-gray-800">{s.scenarioName || `${s.firstName||''} ${s.lastName||''}`.trim() || 'Unnamed'}</div>
                    <div className="text-xs text-gray-500 mt-1">${parseFloat(s.loanAmount||0).toLocaleString()} · {s.loanType||s.loanProgram||'--'} · Income: {s.monthlyIncome ? `$${parseFloat(s.monthlyIncome).toLocaleString()}/mo` : '--'}</div>
                  </button>
                ))}
              </div>}
        </div>
