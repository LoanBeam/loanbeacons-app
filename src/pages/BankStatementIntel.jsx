      ...
      <div className="min-h-screen bg-slate-50" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
        ...
        <div className="bg-gradient-to-br from-slate-900 to-violet-950 px-6 py-10">
          <div className="max-w-2xl mx-auto">
            <button onClick={() => navigate('/')} className="flex items-center gap-1.5 text-violet-300 hover:text-white text-xs font-semibold mb-6 transition-colors">← Back to Dashboard</button>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-11 h-11 bg-violet-500 rounded-2xl flex items-center justify-center text-white font-black text-sm shadow-lg shadow-violet-900/40">07</div>
              <div>
                <span className="text-xs font-bold tracking-widest text-violet-400 uppercase">Stage 1 — Pre-Structure &amp; Initial Analysis</span>
                <h1 style={{ fontFamily: "'DM Serif Display', Georgia, serif" }} className="text-2xl font-normal text-white mt-0.5">Bank Statement Intelligence™</h1>
              </div>
            </div>
            <p className="text-violet-300 text-sm leading-relaxed mb-5">Non-QM income qualification via 12 or 24 months of bank statements. AI extraction, expense ratio engine, and underwriter-ready letters.</p>
            <div className="flex flex-wrap gap-2">
              {['12 / 24-Month Analysis', 'AI PDF Extraction', 'Expense Ratio Engine', 'Business vs Personal', 'NSF Flags', 'Underwriter Letter'].map(tag => (
                <span key={tag} className="text-xs bg-white/10 border border-white/10 text-violet-200 px-3 py-1 rounded-full font-medium">{tag}</span>
              ))}
            </div>
          </div>
        </div>
        <div className="max-w-2xl mx-auto px-6 py-8">
          ...
          {scenarios.length === 0 ? (
            ...
            <div className="text-center py-10 bg-white rounded-3xl border border-slate-100 shadow-sm">
              ...
              <button onClick={() => setSearch('')} className="mt-2 text-xs text-violet-500 hover:underline">Clear search</button>
            </div>
          ) : (
            <div className="space-y-2.5">
              ...
              {displayed.map(s => {
                ...
                return (
                  <button key={s.id} onClick={() => navigate('/bank-statement-intel?scenarioId=' + s.id)}
                    className="w-full text-left bg-white border border-slate-200 rounded-2xl px-5 py-4 hover:border-violet-300 hover:shadow-md hover:bg-violet-50/30 transition-all group">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-2 mt-1.5">
                          ...
                          {s.creditScore && <span className="text-xs bg-violet-50 text-violet-600 border border-violet-100 px-2 py-0.5 rounded-full font-mono">FICO {s.creditScore}</span>}
                        </div>
                      </div>
              ...
              {hasMore && (
                <button onClick={() => setShowAll(true)} className="w-full text-center text-xs font-bold text-violet-500 hover:text-violet-700 py-3 border border-dashed border-violet-200 rounded-2xl hover:bg-violet-50 transition-all">
