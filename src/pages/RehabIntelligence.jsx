...
export default function RehabIntelligence() {
  ...
  const handleSaveToRecord = async () => {
    ...
    try {
      const writtenId = await reportFindings('REHAB_INTEL', {
        product: selectedProductId,
        rehabCost: summary.rehabCost,
        aiv: summary.aivData?.aiv || null,
        loanPurpose: summary.loanPurpose,
        timestamp: new Date().toISOString(),
      });
      if (writtenId) setSavedRecordId(writtenId);
    } catch(e) {
      console.error('Rehab DR save failed:', e);
    } finally {
  ...
  return (
    <div style={styles.container}>
      {/* Always-visible Decision Record Banner when scenario is loaded */}
      {scenarioIdParam && (
