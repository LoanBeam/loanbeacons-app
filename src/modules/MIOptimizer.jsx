...
function MIOptimizer() {
  ...
  const { primarySuggestion, secondarySuggestions, logFollow, logOverride } =
    useNextStepIntelligence({
      currentModuleKey:       'MI_OPTIMIZER',
      loanPurpose,
      decisionRecordFindings: {
        MI_OPTIMIZER: {
          analysisRun:    showResults,
      ...
      <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-3xl mx-4 mt-4 mb-6">
        <div className="relative p-8">
          <div className="flex items-start justify-between flex-wrap gap-6">
            <div style={{ flex: 1 }}>
              <p className="text-orange-400 text-xs font-bold uppercase tracking-widest mb-1.5">Module 16 · Stage 2: Lender Fit</p>
              <h1 style={{ fontFamily: "'DM Serif Display', Georgia, serif" }} className="text-3xl font-bold text-white mb-2">MI Optimizer™</h1>
              <p className="text-slate-400 text-sm max-w-xl leading-relaxed">Compare Monthly, Single, Split, and Lender-Paid MI · Find the lowest true cost for every LTV and FICO</p>
            </div>
            <div className="bg-slate-800/60 border border-slate-700 rounded-2xl px-5 py-4 flex-shrink-0" style={{ minWidth: '240px' }}>
              ...
              <button onClick={() => { setSelectedScenario(null); setScenarioId(''); setShowResults(false); }}
                className="text-xs text-slate-400 hover:text-white mt-2 block transition-colors">Change scenario →</button>
            </div>
          </div>
        </div>
      </div>
