...
export default function DPAIntelligence() {
  ...
  useEffect(() => {
    ...
    getDoc(doc(db, 'scenarios', scenarioId))
      .then(snap => {
        if (snap.exists()) setScenarioData({ id: snap.id, ...snap.data() });
      })
  ...
  const scenario = useMemo(() => {
    ...
    // annualIncome: Firestore stores monthlyIncome, multiply by 12
    const annualIncome = d.monthlyIncome
      ? Math.round(d.monthlyIncome * 12)
      : parseFloat(searchParams.get('annualIncome')) || 0;
    // backendDTI: Firestore stores backDti as a percentage (46.03), convert to decimal
    const backendDTI = d.backDti
      ? d.backDti / 100
      : parseFloat(searchParams.get('backendDTI')) || 0;
    // purchasePrice: Firestore stores propertyValue
    const purchasePrice = parseFloat(d.propertyValue || d.purchasePrice || searchParams.get('purchasePrice')) || 0;
    // occupancy: normalize "Primary Residence" → "primary"
    const rawOcc = d.occupancy || searchParams.get('occupancy') || 'primary';
    const occupancy = rawOcc.toLowerCase().includes('primary') ? 'primary'
      : rawOcc.toLowerCase().includes('second') ? 'secondary'
      : rawOcc.toLowerCase().includes('invest') ? 'investment'
      : 'primary';
    // firstTimeBuyer: not in Firestore — use override if set, else default false
    const firstTimeBuyer = fthbOverride !== null ? fthbOverride : (d.firstTimeBuyer ?? (searchParams.get('firstTimeBuyer') === 'true') ?? false);

    return {
      scenarioId,
      firstName:      d.firstName      || searchParams.get('firstName')      || '',
      lastName:       d.lastName       || searchParams.get('lastName')        || '',
      streetAddress:  d.streetAddress  || searchParams.get('streetAddress')  || '',
      city:           d.city           || searchParams.get('city')            || '',
      state:          d.state          || searchParams.get('state')           || 'GA',
      zipCode:        d.zipCode        || searchParams.get('zipCode')         || '',
      county:         d.county         || searchParams.get('county')          || '',
      loanType:       d.loanType       || searchParams.get('loanType')        || 'FHA',
      purchasePrice,
      loanAmount:     parseFloat(d.loanAmount  || searchParams.get('loanAmount'))  || 0,
      creditScore:    parseInt(d.creditScore   || searchParams.get('creditScore')) || 0,
      annualIncome,
      householdSize:  parseInt(d.householdSize || searchParams.get('householdSize')) || 1,
      firstTimeBuyer,
      backendDTI,
      occupancy,
      lenderId:       d.lenderId    || searchParams.get('lenderId')    || '',
      lenderName:     d.lenderName  || searchParams.get('lenderName')  || '',
    };
  }, [scenarioData, searchParams, scenarioId, fthbOverride]);
  ...
  // ── Load lender approval states (Phase 1: read-only) ─────────────────────
  useEffect(() => {
    if (!scenario.lenderId || !auth.currentUser) return;
    setLoadingApprovals(true);
    const q = query(
      collection(db, 'dpa_lender_approvals'),
      where('lo_id',     '==', auth.currentUser.uid),
      where('lender_id', '==', scenario.lenderId)
    );
    getDocs(q).then(snap => {
      const map = {};
  ...
  }, [scenario.lenderId, auth.currentUser]);
  ...
  const handleRunSearch = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      // Fire Haiku summaries for PASS programs
  ...
  }, [brokerOnly, scenario, scenarioId, reportFindings]);
...
${evaluation.warnings?.length ? 'Warning: ' + evaluation.warnings[0] : ''}`;
  ...
  const handleRequestApproval = async (program) => {
    if (!auth.currentUser || !scenario.lenderId) return;
    setRequestingId(program.id);
    try {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-[#1B3A6B] px-6 py-3">
        <div className="max-w-7xl mx-auto">
          ...
          {scenarioLoading ? (
            <div className="h-5 w-64 bg-blue-800 rounded animate-pulse" />
          ) : (
          <div className="flex flex-wrap items-center gap-x-6 gap-y-1">
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-blue-100">
              ...
              <span className={scenario.firstTimeBuyer ? 'text-emerald-300 font-semibold' : 'text-blue-200'}>
                ...
                {fthbOverride !== null && <span className="text-amber-300 text-[10px] ml-1">(override)</span>}
              </span>
            </div>
          </div>
          )}
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 pb-24 space-y-5">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={() => setFthbOverride(v => v === null ? !scenario.firstTimeBuyer : !v)}
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${scenario.firstTimeBuyer ? 'bg-emerald-600' : 'bg-gray-300'}`}
              >
                <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${scenario.firstTimeBuyer ? 'translate-x-6' : 'translate-x-1'}`} />
              </button>
              <div>
                <p className="text-sm font-semibold text-gray-800">First-Time Homebuyer</p>
                <p className="text-xs text-gray-500">
                  {scenario.firstTimeBuyer ? 'FTHB programs included' : 'Toggle on if borrower is a first-time buyer'}
                  {fthbOverride !== null && <span className="text-amber-600 font-medium"> · Manual override</span>}
                </p>
              </div>
            </div>
          ...
          <div className="space-y-3">
            {results.map(({ program, evaluation }) => (
              <ProgramCard
                ...
                approvalState={approvalMap[program.id] || APPROVAL_STATES.UNKNOWN}
                lenderName={scenario.lenderName}
                lenderId={scenario.lenderId}
                onSelect={() => setSelectedProgram({ program, evaluation })}
                onRequestApproval={() => handleRequestApproval(program)}
                requestingApproval={requestingId === program.id}
      ...
      {selectedProgram && (
        <ProgramDrawer
          ...
          approvalState={approvalMap[selectedProgram.program.id] || APPROVAL_STATES.UNKNOWN}
          lenderName={scenario.lenderName}
          onRequestApproval={() => handleRequestApproval(selectedProgram.program)}
          requestingApproval={requestingId === selectedProgram.program.id}
          onClose={() => setSelectedProgram(null)}
        />
      )}

      {/* ── CANONICAL SEQUENCE BAR ────────────────────────────────────────── */}
      <CanonicalSequenceBar
...
// ── PROGRAM CARD ─────────────────────────────────────────────────────────────
function ProgramCard({ program, evaluation, haiku, haikusLoading, approvalState, lenderName, lenderId, onSelect, onRequestApproval, requestingApproval, showFailDetail, onToggleFailDetail, brokerOnly }) {
...
// ── LENDER APPROVAL BADGE ─────────────────────────────────────────────────────
function LenderApprovalBadge({ approvalState, lenderName, lenderId, onRequestApproval, requesting, compact = false }) {
  if (!lenderId) {
    return (
    ...
    <div className="flex items-center gap-2 flex-wrap">
      ...
      {!compact && (
        <button
          ...
          disabled={requesting}
          className="text-[11px] text-[#1B3A6B] hover:text-blue-800 font-semibold underline underline-offset-2 disabled:opacity-50"
        >
...
function ProgramDrawer({ program, evaluation, haiku, approvalState, lenderName, onRequestApproval, requestingApproval, onClose }) {
  const cfg = STATUS_CONFIG[evaluation.status];
  const typeCfg = TYPE_LABELS[program.program_type] || { label: program.program_type, color: 'bg-gray-100 text-gray-700' };
  const freshness = getFreshnessLabel(program.last_verified_date);
          ...
          <div>
            ...
            {approvalState === APPROVAL_STATES.UNKNOWN && (
              <p className="text-xs text-gray-400 mt-1.5">
                Requesting approval will notify your AE via the AE Share Service with the program details pre-filled.
              </p>
            )}
          </div>
