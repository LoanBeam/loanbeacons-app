...
export default function VAIRRRLIntelligence() {
  ...
  const handleSaveToRecord = async () => {
    ...
    try {
      const writtenId = await reportFindings("VA_IRRRL", {
        ntbRate, ntbRecoup, ntbOK,
        curRate, propRate,
        rateDrop: parseFloat(rateDrop.toFixed(3)),
        curPI: parseFloat(curPI.toFixed(2)),
        newPI: parseFloat(newPI.toFixed(2)),
        piSavings: parseFloat(piSavings.toFixed(2)),
        pricingMethod: pr.method,
        credit: parseFloat(credit.toFixed(2)),
        grossFees: parseFloat(grossFees.toFixed(2)),
        netFees: parseFloat(netFees.toFixed(2)),
        cashToClose: parseFloat(cashToClose.toFixed(2)),
