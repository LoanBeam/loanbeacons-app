...
export default function DPAIntelligence() {
  ...
  const handleRunSearch = async () => {
    ...
    } catch (e) { console.error('[DPA] lenderProfiles read failed:', e); }
  ...
  const generateHaikus = async (programs) => {
    ...
    await Promise.all(programs.map(async ({ program, evaluation }) => {
      try {
        ...
        const r = await fetch('/anthropic-api/v1/messages', {
          method: 'POST',
  ...
  const handleOpenAeModal      = useCallback((program, evaluation) => { setAeSent(false); setAeSending(false); setAeShareModal({ program, evaluation }); }, []);
  const handleCloseAeModal     = useCallback(() => { setAeShareModal(null); setAeSent(false); setAeSending(false); }, []);
  const handleOpenProgramShare = useCallback((program, evaluation) => { setProgramShareModal({ program, evaluation }); setProgSent(false); }, []);
  ...
  const handleProgramSend = useCallback(async (emails, shareType, message) => {
    ...
    setProgSending(true);
    try {
      const fitReasons = (evaluation.steps || []).filter(s => s.pass === true).map(s => s.reason || s.name).filter(Boolean).slice(0, 5);
      await addDoc(collection(db, 'scenarioShares'), {
        scenarioId,
        aeEmails:  emails,
        shareType: shareType || 'SCENARIO_REVIEW',
        message:   message || '',
        status:    'pending',
        ...
        dpaContext: {
          ...
          dpaAmount:     evaluation.dpa_amount_calculated ? `$${evaluation.dpa_amount_calculated.toLocaleString()}` : program.assistance_pct ? `${(program.assistance_pct * 100).toFixed(1)}% of purchase price` : null,
          ltvLimit:      evaluation.cltv_details?.program_max ? `${(evaluation.cltv_details.program_max * 100).toFixed(1)}%` : null,
          ...
          cltv:          evaluation.cltv_details?.cltv_with_dpa ? `${(evaluation.cltv_details.cltv_with_dpa * 100).toFixed(1)}%` : null,
        },
        moduleContext: { moduleName: 'DPA Intelligence™', moduleNumber: '07' },
      });
      setProgSent(true);
      setTimeout(() => handleCloseProgramShare(), 2500);
    } catch (err) { console.error('[DPA] program share failed:', err); }
    finally { setProgSending(false); }
  }, [programShareModal, scenarioId, brokerageLenderName, effectiveLenderName, auth.currentUser, handleCloseProgramShare]);
    ...
    try {
      ...
      if (scenario.lenderId) {
        await addDoc(collection(db, 'dpa_lender_approvals'), {
          ...
          approval_state: APPROVAL_STATES.REQUESTED, requested_at: serverTimestamp(), ae_outreach_id: null, last_updated: serverTimestamp(),
        });
        setApprovalMap(prev => ({ ...prev, [program.id]: APPROVAL_STATES.REQUESTED }));
      }
      setAeSent(true);
      setTimeout(() => handleCloseAeModal(), 2000);
    } catch (err) { console.error('AE send failed:', err); }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-[#1B3A6B] px-6 py-3">
        <div className="max-w-7xl mx-auto">
          <p className="text-[11px] font-semibold text-blue-300 uppercase tracking-widest mb-1">Borrower Scenario — DPA Intelligence™</p>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-5">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-3">
                <button onClick={() => setBrokerOnly(v => !v)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${brokerOnly ? 'bg-[#1B3A6B]' : 'bg-gray-300'}`}>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">{brokerOnly ? 'Showing broker-eligible programs — toggle off to see all' : 'Showing all programs including retail-only'}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <button onClick={() => setFthbOverride(v => v === null ? true : v === true ? false : null)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${effectiveFthb ? 'bg-[#1B3A6B]' : 'bg-gray-300'}`}>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">{fthbOverride === null ? `FTHB ${scenario.firstTimeBuyer ? 'detected from scenario' : 'not detected'}` : fthbOverride ? 'FTHB programs included · Manual override' : 'FTHB programs excluded · Manual override'}</p>
                </div>
              </div>
            </div>
            <div className="flex flex-wrap gap-2 text-xs">
              {[scenario.state && { label: 'State', value: scenario.state }, scenario.county && { label: 'County', value: scenario.county }, scenario.loanType && { label: 'Loan', value: scenario.loanType }, scenario.creditScore > 0 && { label: 'FICO', value: scenario.creditScore }].filter(Boolean).map(({ label, value }) => (
                <span key={label} className="bg-gray-100 text-gray-700 px-2 py-1 rounded-md"><span className="text-gray-400">{label}: </span>{value}</span>
              ))}
            </div>
            <button onClick={handleRunSearch} disabled={isRunning} className="bg-[#1B3A6B] hover:bg-blue-800 disabled:bg-gray-300 text-white font-semibold text-sm px-6 py-2.5 rounded-lg transition-colors flex items-center gap-2 whitespace-nowrap">
              {isRunning ? (<><svg className="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" /></svg>Running Stack Optimizer™…</>) : (<><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>Run DPA Intelligence™</>)}
            </button>
          </div>
        </div>
        ...
        {hasRun && (
          <div className="grid grid-cols-3 gap-3">
            {[{ label: 'Eligible', count: passCount, color: 'emerald' }, { label: 'Conditional', count: condCount, color: 'amber' }, { label: 'Ineligible', count: failCount, color: 'red' }].map(({ label, count, color }) => (
        ...
        {!hasRun && !isRunning && (
          <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
            ...
            <p className="text-sm text-gray-500 max-w-sm mx-auto">Run DPA Intelligence™ to evaluate {brokerOnly ? 'broker-eligible' : 'all'} programs against this borrower's profile using the Stack Optimizer™.</p>
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full max-w-lg mx-4 bg-[#0d1117] rounded-2xl shadow-2xl overflow-hidden border border-[#21262d]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#21262d]">
              <div>
                <p className="text-[11px] font-semibold text-[#8b949e] uppercase tracking-widest mb-0.5">Share Program with AE — {brokerageLenderName || effectiveLenderName}</p>
                <h2 className="text-white font-bold text-sm leading-snug">{programShareModal.program.program_name}</h2>
                <p className="text-[#8b949e] text-xs mt-0.5">{programShareModal.evaluation.status} · {TYPE_LABELS[programShareModal.program.program_type]?.label}{programShareModal.evaluation.dpa_amount_calculated ? ` · $${programShareModal.evaluation.dpa_amount_calculated.toLocaleString()}` : programShareModal.program.assistance_pct ? ` · ${(programShareModal.program.assistance_pct * 100).toFixed(1)}% of price` : ''}</p>
              </div>
              <button onClick={handleCloseProgramShare} className="text-[#8b949e] hover:text-white transition-colors">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            {programShareModal.evaluation.steps?.filter(s => s.pass === true).length > 0 && (
              <div className="px-6 py-3 border-b border-[#21262d] bg-[#0a1a0a]">
                <p className="text-[10px] font-semibold text-emerald-400 uppercase tracking-widest mb-2">Why This Program Fits</p>
                <div className="space-y-1">
                  {programShareModal.evaluation.steps.filter(s => s.pass === true).slice(0, 4).map((step, i) => (
                    <div key={i} className="flex items-start gap-2">
                      <svg className="w-3 h-3 text-emerald-400 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg>
                      <p className="text-[11px] text-emerald-300">{step.reason || step.name}</p>
                    </div>
                  ))}
                </div>
                {programShareModal.evaluation.warnings?.length > 0 && (
                  <div className="mt-2 space-y-1">{programShareModal.evaluation.warnings.map((w, i) => <p key={i} className="text-[11px] text-amber-400">⚠️ {w}</p>)}</div>
                )}
              </div>
            )}
    ...
    <div className={`bg-white rounded-xl border ${isFail ? 'border-gray-200 opacity-75' : cfg.border} shadow-sm overflow-hidden`}>
      ...
      {!isFail && (
        <div className="px-4 py-2 border-t border-gray-100">
          {haikusLoading && !haiku ? <div className="h-4 bg-gray-100 rounded animate-pulse w-3/4" /> : haiku ? <p className="text-xs text-gray-600 italic">{haiku}</p> : null}
        </div>
      )}
      {!isFail && evaluation.cltv_details && (
        <div className="px-4 py-2 bg-gray-50 border-t border-gray-100 flex flex-wrap gap-4 text-xs text-gray-600">
          ...
          <span className={`font-medium ${freshness.color === 'green' ? 'text-emerald-600' : freshness.color === 'amber' ? 'text-amber-600' : 'text-red-500'}`}>{freshness.urgent ? '⚠️ ' : '✓ '}{freshness.label}</span>
        </div>
      )}
      {evaluation.warnings?.length > 0 && (
        <div className="px-4 py-2 bg-amber-50 border-t border-amber-100">
          {evaluation.warnings.map((w, i) => <p key={i} className="text-xs text-amber-700">⚠️ {w}</p>)}
        </div>
      )}
      {isFail && (
        <div className="px-4 py-2 border-t border-gray-100">
          ...
          {showFailDetail && <div className="mt-1.5 space-y-1">{evaluation.fail_reasons?.map((r, i) => <p key={i} className="text-xs text-red-600">✗ {r}</p>)}</div>}
        </div>
      )}
      <div className="px-4 py-3 border-t border-gray-100 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {!isFail && lenderId && approvalState === 'unknown' && (
            <button onClick={onRequestApproval} className="inline-flex items-center gap-1 text-[11px] font-semibold text-[#1B3A6B] hover:text-blue-800 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-full px-2.5 py-1 transition-colors">
...
function LenderApprovalBadge({ approvalState, lenderName, lenderId, onShareWithAe }) {
  ...
  if (approvalState === APPROVAL_STATES.APPROVED) {
    return (
      <button onClick={onShareWithAe} title={`Click to share this program with your AE at ${name}`} className="group inline-flex items-center gap-1.5 text-[11px] font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 hover:border-emerald-400 rounded-full px-2.5 py-1 transition-colors cursor-pointer">
        <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/></svg>
        Approved — {name}
        <svg className="w-2.5 h-2.5 opacity-60 group-hover:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
      </button>
    );
  }
    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-xl bg-white shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          <div><h4 className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Program Description</h4><p className="text-sm text-gray-700 leading-relaxed">{program.description}</p></div>
          <div><h4 className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Contact & Resources</h4>
            <div className="space-y-1.5">
              ...
              {program.contact_email && <p className="text-sm text-gray-700"><span className="text-gray-400">Email: </span><a href={`mailto:${program.contact_email}`} className="text-[#1B3A6B] font-medium">{program.contact_email}</a></p>}
            </div>
          </div>
          <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
            ...
            <div className="flex-1"><p className="text-xs font-semibold text-gray-700">{freshness.label}</p><p className="text-xs text-gray-400">Source: {program.source} · Confidence: {confidence.label}</p></div>
          </div>
          <div><h4 className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Stack Optimizer™ — Eligibility Trace</h4>
          ...
          <div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {approvalState === APPROVAL_STATES.UNKNOWN && (
                <button onClick={onRequestApproval} className="inline-flex items-center gap-1.5 text-[12px] font-semibold text-white bg-[#1B3A6B] hover:bg-blue-800 rounded-lg px-3 py-1.5 transition-colors">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                  Request Approval via AE
                </button>
              )}
            </div>
            {approvalState === APPROVAL_STATES.UNKNOWN && <p className="text-xs text-gray-400 mt-1.5">Sends program details and scenario to your AE via the AE Share Service.</p>}
          </div>
          {program.stacking_rules?.subordinate_financing_rules && (
            <div><h4 className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1.5">Stacking Rules</h4>
              <p className="text-xs text-gray-600">{program.stacking_rules.subordinate_financing_rules}</p>
              {program.stacking_rules.mi_impact_rules && <p className="text-xs text-gray-500 mt-1">MI: {program.stacking_rules.mi_impact_rules}</p>}
            </div>
          )}
        </div>
      </div>
