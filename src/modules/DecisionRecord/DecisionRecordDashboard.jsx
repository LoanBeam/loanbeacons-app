...
const LTC={FHA:{bg:'bg-blue-100',text:'text-blue-800',dot:'#3b82f6'},CONVENTIONAL:{bg:'bg-violet-100',text:'text-violet-800',dot:'#7c3aed'},VA:{bg:'bg-green-100',text:'text-green-800',dot:'#16a34a'},USDA:{bg:'bg-emerald-100',text:'text-emerald-800',dot:'#059669'},'NON-QM':{bg:'bg-orange-100',text:'text-orange-800',dot:'#ea580c'},DEFAULT:{bg:'bg-slate-100',text:'text-slate-600',dot:'#94a3b8'}};
...
function RecordRow({record}){
  ...
  return(<>
    <tr onClick={()=>setOpen(v=>!v)} className={`border-b border-slate-100 cursor-pointer transition-colors ${open?'bg-amber-50':'hover:bg-slate-50'} group`} style={{borderLeft:`3px solid ${riskLeft}`}}>
      ...
      <td className="pl-2 pr-4 py-3 w-10 text-right"><span className={`text-slate-400 text-xs transition-transform inline-block ${open?'rotate-90':''}`}>▶</span></td>
    </tr>
...
export default function DecisionRecordDashboard(){
  ...
  const named=records.filter(r=>getBorrowerName(r));
  const unnamed=records.filter(r=>!getBorrowerName(r));
  ...
  const counts={all:named.length,attention:highRisk.length,draft:named.filter(r=>getStatus(r)==='draft').length,submitted};
    ...
    <div className="min-h-screen bg-slate-50" style={{fontFamily:"'DM Sans', system-ui, sans-serif"}}>
      ...
      <div className="max-w-7xl mx-auto px-6 py-5">
        <div className="flex items-center gap-3 mb-5 flex-wrap">
          {[{key:'all',label:'All Deals',count:counts.all},{key:'attention',label:'⚑ Needs Attention',count:counts.attention},{key:'draft',label:'In Progress',count:counts.draft},{key:'submitted',label:'Submitted',count:counts.submitted}].map(f=>(
            <button key={f.key} onClick={()=>setFilter(f.key)} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold border transition-all ${filter===f.key?(f.key==='attention'?'bg-red-500 border-red-500 text-white shadow-md':'bg-amber-500 border-amber-500 text-white shadow-md'):'bg-white border-slate-200 text-slate-600 hover:border-amber-300 hover:text-amber-700'}`}>
        ...
        {loading?(
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-16 text-center"><div className="text-5xl mb-4">📋</div><div className="text-slate-700 font-semibold text-lg">No records match this filter</div><div className="text-slate-400 text-sm mt-2">Records are auto-created when you save any module to the Decision Record.</div></div>
        ):(
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            {unnamed.length>0&&(
              <div className="border-t border-slate-100 px-5 py-3">
                ...
                {showUnnamed&&<table className="w-full mt-3"><tbody>{unnamed.map(record=><RecordRow key={record.id} record={record}/>)}</tbody></table>}
              </div>
            )}
          </div>
        )}

        {filtered.length>0&&<p className="text-center text-xs text-slate-400 mt-4">{filtered.length} deal{filtered.length!==1?'s':''} · Click any row to expand and launch modules{unnamed.length>0?` · ${unnamed.length} unnamed below`:''}</p>}
      </div>
    </div>
  );
}
