...
export const MODULE_REGISTRY = {
  ...
  INCOME_ANALYSIS:           { label: 'Income Analysis',                 route: '/income-analyzer',          stage: 1 },
  ...
  DPA_INTELLIGENCE:          { label: 'DPA Intelligence',                route: '/dpa-intelligence',         stage: 2 },
  AUS_RESCUE:                { label: 'AUS Rescue',                      route: '/aus-rescue',               stage: 2 },
  DEBT_CONSOLIDATION_INTEL:  { label: 'Debt Consolidation Intelligence', route: '/debt-consolidation',       stage: 2 },
  ...
  CRA_ELIGIBILITY:           { label: 'CRA Eligibility Intelligence',    route: '/cra-eligibility',          stage: 2 },
  RATE_BUYDOWN_CALCULATOR:   { label: 'Rate Buydown Calculator',         route: '/rate-buydown',             stage: 3 },
  PROPERTY_INTEL:            { label: 'Collateral Intelligence',         route: '/property-intel',           stage: 3 },
  CLOSING_COST_CALCULATOR:   { label: 'Closing Cost Calculator',         route: '/closing-cost-calc',        stage: 3 },
  MI_OPTIMIZER:              { label: 'MI Optimizer',                    route: '/mi-optimizer',             stage: 3 },
  ARM_STRUCTURE_INTEL:       { label: 'ARM Structure Intelligence',      route: '/arm-structure',            stage: 3 },
  REHAB_INTEL:               { label: 'Rehab Intelligence',              route: '/rehab-intelligence',       stage: 3 },
  PIGGYBACK_2ND_OPTIMIZER:   { label: 'Piggyback 2nd Optimizer',         route: '/piggyback-optimizer',      stage: 3 },
  RATE_INTEL:                { label: 'Rate Intelligence',               route: '/rate-intel',               stage: 3 },
  TITLE_INTEL:               { label: 'Title Intelligence',              route: '/title-intel',              stage: 4 },
  DISCLOSURE_INTEL:          { label: 'Disclosure Intelligence',         route: '/disclosure-intel',         stage: 4 },
  COMPLIANCE_INTEL:          { label: 'Compliance Intelligence',         route: '/compliance-intel',         stage: 4 },
  FLOOD_INTEL:               { label: 'Flood Intelligence',              route: '/flood-intel',              stage: 4 },
...
function runRulesEngine({ currentModuleKey, loanPurpose, findings, scenarioData, completedModules }) {
  ...
  if (currentModuleKey === 'SCENARIO_CREATOR') {
    if (loanPurpose === 'purchase') {
      ...
      raw.push(suggest('INCOME_ANALYSIS',   'Cash-out triggers full income documentation review under cash-out guidelines.', 'MEDIUM'));
  ...
  if (currentModuleKey === 'QUALIFYING_INTEL') {
    ...
    if (dti > 45) {
      raw.push(suggest('DEBT_CONSOLIDATION_INTEL',
      ...
      raw.push(suggest('INCOME_ANALYSIS',
        'DTI is within range. Proceed to full income documentation analysis.',
        'LOW'));
    }
  }

  if (currentModuleKey === 'INCOME_ANALYSIS') {
    const incomeType      = (f.incomeType || '').toLowerCase();
    const incomeSufficient = f.incomeSufficient !== false; // true unless explicitly false
  ...
  if (currentModuleKey === 'CREDIT_INTEL') {
    ...
    if (score > 0 && score < 580) {
      // Block all program modules — no navigation suggestion, just flag
      raw.push({
  ...
  if (currentModuleKey === 'LENDER_MATCH') {
    ...
    if (!matchFound) {
      ...
      raw.push(suggest('DPA_INTELLIGENCE',
  ...
  if (currentModuleKey === 'DPA_INTELLIGENCE') {
    ...
    if (grantFound && nearAmiLimit) {
      raw.push(suggest('INCOME_ANALYSIS',
        'Grant found but income is near the AMI limit. Re-run Income Analysis to confirm AMI eligibility holds before committing to this program.',
        'HIGH'));
    } else if (grantFound) {
      raw.push(suggest('AUS_RESCUE',
  ...
  if (currentModuleKey === 'AUS_RESCUE') {
    ...
    if (blocker === 'LTV') {
      raw.push(suggest('PROPERTY_INTEL',
        ...
        'HIGH'));
      raw.push(suggest('RATE_BUYDOWN_CALCULATOR',
  ...
  if (currentModuleKey === 'DEBT_CONSOLIDATION_INTEL') {
  ...
  if (currentModuleKey === 'PROPERTY_INTEL') {
    ...
    if ((flipDays < 0 || flipDays >= 91) && appraisalRisk !== 'HIGH' && !wellSeptic) {
      raw.push(suggest('CLOSING_COST_CALCULATOR',
        'Property report clean. Proceed to Closing Cost Calculator.',
        'LOW'));
    }
  }

  if (currentModuleKey === 'CLOSING_COST_CALCULATOR') {
  ...
  if (currentModuleKey === 'ARM_STRUCTURE_INTEL') {
    raw.push(suggest('LENDER_MATCH',
  ...
  if (currentModuleKey === 'COMPLIANCE_INTEL') {
    ...
    if (aprSpread) {
      raw.push(suggest('RATE_BUYDOWN_CALCULATOR',
        'APR spread flag detected — submission is blocked. Return to Rate Buydown Calculator to restructure rate and points.',
        'HIGH', { canSkip: false }));
    } else if (hpml) {
      raw.push(suggest('DISCLOSURE_INTEL',
